...
impl ExecutorMetrics {
      ...
      B: reth_primitives_traits::Block,
  {
      ...
      let log_path = std::env::var("BLOCK_EXECUTION_LOG_FOLDER")
          .unwrap_or_else(|_| "/var/log".to_string())
          + "/execution-time.jsonl";

      // Store traces in a rotating file
      let mut log = FileRotate::new(
          log_path.clone(),
          AppendTimestamp::default(FileLimit::MaxFiles(2)),
          ContentLimit::Lines(3900),
          Compression::None,
          #[cfg(unix)]
          None,
      );

      let block_info_json = serde_json::json!({
          "duration": execute_start.elapsed().as_nanos(),
          "gasUsed": block.header().gas_used(),
          "blockNumber": block.header().number(),
          "hash": format!("{:?}", block.header().hash_slow()),
          "parentHash": format!("{:?}", block.header().parent_hash()),
      }).to_string();

      writeln!(log, "{}", block_info_json).unwrap();
